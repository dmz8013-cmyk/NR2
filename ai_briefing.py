...
5. 통계·여론조사: 조사기관과 조사기간을 병기할 것. 둘 중 하나라도 원문에 없으면 해당 수치·조사 내용을 항목에서 빼고 확인된 사실만 쓸 것 ("~로 전해졌다" 류 완곡 전언 금지).
...
def generate_briefing_with_ai(
    ...
    # 항목 종결 허용 술어 — config/briefing_layout.json 한 곳에서 관리
    try:
...
2. 출처·참고 링크 블록은 시스템이 발송 시 자동 삽입 — 본문에 '출처:'·URL 줄을
   절대 쓰지 말 것.
3. 제목 다음 한 줄 빈 줄 후 4개 분야(🏛️ 정치/시사, 💰 경제/산업, 🤖 AI/기술, 🎯 기타) 시작.
   섹션 헤더 아래에 구분선(─────)을 긋지 말 것.
4. [포맷] 각 분야별 핵심 내용은 ▪ 고밀도 1줄 항목으로 작성:
   - [줄 규칙 — 최우선] 각 ▪ 항목은 반드시 '새 줄'에서 시작하고,
     ▪ 항목과 다음 ▪ 항목 사이에 빈 줄을 넣지 말 것(연속된 줄로 붙일 것).
     섹션 제목 다음에 빈 줄 1개, 그 다음 줄부터 ▪ 시작. 한 줄에 ▪ 두 개 이상 금지.
     맥락 줄("→")도 절대 쓰지 말 것.
   ...
   - [문체] 통신사 데스크 톤 — 명사형·음슴체 종결, 건조하고 전문적으로.
     ...
     무정보 필러 절도 금지: '추후 공개될 예정', '별도 공시될 예정',
     '향후 절차가 계속 진행될 예정', '~ 진행 중', '안정화가 진행' 등
     새 정보 없이 길이만 채우는 마무리 절을 붙이지 말 것.
     전언·풍문 술어 절대 금지: '전해졌다/전해짐/전해진다', '알려졌다/알려짐',
     '~로 보인다', '~는 관측' — 확인된 사실만 쓸 것.
     항목 종결은 명사형(…음/됨/임) 또는 사실 서술 술어로 통일
     (허용 예: {allowed_endings_str}).
...
8. 마지막 분야 요약으로 본문을 끝낼 것 — 출처 푸터는 시스템이 자동 삽입하므로
   본문 끝에 '출처:'·URL 줄을 쓰지 말 것.
...
15. [섹션 고정] 암호화폐·코인 관련 항목(시세·ETF·규제·거래소 포함)은 예외 없이 💰 경제/산업 섹션에만 배치하고, 브리핑 전체에서 ▪ 항목 기준 최대 1건만 다루세요(별도 제공되는 [시세 데이터] 줄은 이 제한과 무관). 코인 소재가 여러 건이면 파급력이 가장 큰 1건만 선택하세요.
...
BANNED_PREDICATE_RE = re.compile(
    ...
    # 무정보 필러 절: '추후/별도/향후 ~ 예정·공개·공시·발표'(간격 12자 이내로
    # '향후 5년간 10조 투자 계획 발표' 같은 확정 사실 오탐 방지), 진행형 마무리
    r"|(?:추후|별도|향후)[^.\n]{0,12}?(?:예정|공개|공시|발표)"
    r"|진행될 예정|진행 중|안정화가 진행"
    # 루머 유통 술어: 전언·풍문·관측성 종결 전면 금지 (2026-09-15)
    r"|전해졌|전해진다|전해짐|전해져|알려졌|알려진다|알려짐|알려져"
    r"|로 보인다|로 보임|는 관측|란 관측|라는 관측")
MAX_ITEM_CHARS = 200
...
- 전언·풍문 술어 금지: '전해졌다/전해짐/전해진다', '알려졌다/알려짐',
  '~로 보인다', '~는 관측' — 완곡 전언으로 바꾸지 말고, 확인된 사실만
  명사형(…음/됨/임) 또는 사실 서술 술어(예: 확정, 체결, 비판, 기각)로 종결.
- 결측 표기('미상', '미정', '미확인', '알려지지 않음', '확인되지 않음') 금지 —
  정보가 없다는 절은 통째로 삭제하고 확인된 사실만 남길 것.
  단, 인명(김미정 등)·'의미상' 같은 정상 어휘면 그대로 둘 것.
- 무정보 필러 절 금지: '추후 공개될 예정', '별도 공시될 예정', '향후 절차가
  계속 진행될 예정', '~ 진행 중', '안정화가 진행' 등 새 정보 없이 문장을
  채우는 마무리 절은 삭제할 것. 삭제하면 항목이 한 문장만 남거나 그 절이
  구체 사실(예: 검찰 수사 진행 등 핵심 정보)이면 원문 그대로 유지.
...
def send_briefing():
    ...
    try:
        ...
        # 3.8) 문체 2차 패스 — verify_pass 수정문·완곡문이 해설 술어/필러 절을
        #      재유입하는 경로 차단 (위반 항목이 있을 때만 1회 재작성, 실패 시 원문)
        try:
            api_key = _env("ANTHROPIC_API_KEY")
            if api_key and anthropic is not None:
                briefing = _enforce_density(
                    briefing, anthropic.Anthropic(api_key=api_key), "")
        except Exception as ed_err:
            logger.warning(f"[문체강제 2차] 실패 — 원문 유지: {ed_err}")

        # 3.9) 공용 레이아웃 — 헤더(출처 2링크)·푸터(참고 2링크)·섹션 간격을
        #      config/briefing_layout.json 템플릿으로 재조립 (구식 출처·nr2 푸터 대체)
        try:

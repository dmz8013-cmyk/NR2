...
# 2줄 구조(헤드라인 90자 + 맥락 100자) × 4분야 × 3~6항목 기준으로 산정
BRIEFING_MIN_CHARS = 1800
BRIEFING_MAX_CHARS = 3000
...
4. [포맷] 각 분야별 핵심 내용은 항목당 2줄 구조로 작성:
   - 1줄차: "▪ " 로 시작하는 팩트 헤드라인 — 90자 이내, 완결 문장
   - 2줄차: "　→ " 로 시작하는 배경·의미·수치 맥락 — 정확히 1문장, 100자 내외
     (맨 앞은 전각 공백 "　" 한 칸 + "→ ")
   - [맥락 줄 원칙] 검증된 사실·보도에 근거한 내용만 쓸 것. 추측성 해석 금지.
     해석·평가를 담아야 하면 반드시 "~로 풀이됨"으로 끝맺을 것.
   - 분야당 3개 이상, 최대 5~6개 항목 (마지막 '기타' 분야도 동일 형식으로 완결)
   - [매우 중요] 같은 분야 안에서는 빈 줄 없이 연속된 줄로 붙여 쓸 것.
     빈 줄은 오직 분야(섹션) 사이에 딱 한 줄만 허용. 아래 형식을 정확히 따를 것:
     🏛️ 정치/시사
     ▪ 첫 번째 항목의 팩트 헤드라인.
     　→ 첫 번째 항목의 배경·의미 맥락 1문장.
     ▪ 두 번째 항목의 팩트 헤드라인.
     　→ 두 번째 항목의 배경·의미 맥락 1문장.

...
def create():
    ...
    if not current_user.is_admin:
        flash('관리자만 공지를 작성할 수 있습니다.', 'error')
...
def edit(news_id):
    ...
    if not current_user.is_admin:
        flash('관리자만 공지를 수정할 수 있습니다.', 'error')
...
def delete(news_id):
    ...
    if not current_user.is_admin:
        flash('관리자만 공지를 삭제할 수 있습니다.', 'error')
        return redirect(url_for('news.view', news_id=news_id))
...
def toggle_active(news_id):
    ...
    if not current_user.is_admin:
        flash('관리자만 공지 상태를 변경할 수 있습니다.', 'error')
        return redirect(url_for('news.view', news_id=news_id))
    ...
    flash(f'공지가 {status}되었습니다.', 'success')
    return redirect(url_for('news.view', news_id=news_id))
